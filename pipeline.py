...
# re-tuned from one place. Structural — they do NOT track the text inside them. (Each
# module keeps its own copy of the widths it uses; these are pipeline.py's.)
SECTION_BAR_W = 60   # ═ bar around each section header in the .txt report
SUMMARY_BAR_W = 80   # ═ bar around the on-screen (terminal) summary box
WRAP_WIDTH    = 80   # width the flowing comma-lists (locators, plugins…) wrap to
...
def step_catalog():
    ...
    by config to the enabled ones, and the GUI derives its toggle list (order + id set) from it —
...
def track_mute_state(track, automated_ids=frozenset()) -> str:
    """Classify a track's mute (Speaker / activator):
      'muted' — statically off: base <Manual Value="false"/> AND not automated → silent for the
                whole song (the actionable, forgotten-mute / dead-weight case).
      'auto'  — the mute is automated: it toggles during the song (intentional arrangement),
                whatever the base value → never flagged, counted or removed as muted.
      ''      — not muted.
...
def step_project_report(xml_text: str, context: Context) -> tuple:
    ...
    PROJECT SUMMARY  — Creator, BPM, time signature, locators, track counts,
    ...
    # separately, and AU is macOS-only — so the same plugin in two formats is two
...
def aggregate_external_plugins(als_files: list[Path], root: Path) -> Path | None:
    ...
    covers (so the scope is clear up front, like the collect report's title box), then:
      • FULL LIST       — every unique external plugin, name padded to the longest + 3 spaces,
                          then its VST2/VST3/AU format (same column rule as the per-project
                          and collect reports).
      • USAGE BY PROJECT — plugins grouped by the set of projects that share them; the format
    ...
    SHARED by both the CLI (main) and the GUI worker — the single source of truth so the two
    ...
    # VST3 in another groups separately — they are separate installs.
    ...
    # ── HEADER — what this report covers ─────────────────────────────────────────
    ...
    out_lines += ['═' * SECTION_BAR_W, '  @ External Plugins List', '═' * SECTION_BAR_W]
    ...
    out_lines += ['═' * SECTION_BAR_W, '  EXTERNAL PLUGINS — FULL LIST', '═' * SECTION_BAR_W]
    for name, fmt in sorted(plugin_projects.keys(), key=lambda x: (x[0].lower(), x[1])):
        out_lines.append(f'  {name:<{GW}}   {fmt}')
    out_lines.append('')

    # ── USAGE BY PROJECT ───────────────────────────────────────────────────────
    # Both halves wrap at 80. The "→ " prefix marks the project half, and its continuations
    # hang 4 so the two halves stay visually separate when both wrap.
    def wrap(items, prefix=''):
        # Breaks on the comma, never inside an item — textwrap.wrap() would split on the
...
def step_remove_muted_tracks(xml_text: str, context: Context) -> tuple:
    ...
    A track whose mute is AUTOMATED is never removed — the mute toggles during the song so the
    track may be audible; that's intentional arrangement (same guard as automated device On/Off).
    """
    to_remove = []
    automated_ids = set(re.findall(r'<PointeeId\s+Value="(\d+)"', xml_text))
    ...
    for track_tag in ["AudioTrack", "MidiTrack"]:
        for t_start, t_end, t_content in find_blocks(xml_text, track_tag):
            ...
            if not re.search(r'<Manual\s+Value="false"', speaker):
                continue
            # Guard — skip if the mute is automated: the track toggles during the song and may be
            # audible, so removing it would destroy arrangement. Mirrors remove_disabled_devices'
            # automated-On/Off guard.
            at = re.search(r'<AutomationTarget\s+Id="(\d+)"', speaker)
            if at and at.group(1) in automated_ids:
                continue
            to_remove.append({"start": t_start, "end": t_end, "name": track_of(t_start, tracks)})
