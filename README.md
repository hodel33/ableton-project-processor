...
> 🛡️ **Built-in guard** — you can't accidentally nuke the whole thing: if climbing those levels would land on your **drive root** (like `C:\`) or your **user home folder** — say your project sits near the top of the drive — the tool refuses to treat something that broad as home and simply switches the feature **off** for that project. So the safe rule stays simple: **set it to the number of levels up to your song's own top folder, and no higher.** Usually that's `1`.
